...
public class ContextualCodeEditor extends CodeEditor
  ...
  private int defaultInputType(
      boolean typeClassText,
      boolean typeTextFlagMultiLine,
      boolean typeTextFlagNoSuggestions,
      boolean typeTextVariationVisiblePassword) {
    int flags = 0;
    if (typeClassText) {
  ...
  private void convertSelectionCaseInternal(CaseHandler.CaseType caseType) {
    ...
    if (cursor.isSelected()) {
      ...
      if (length > 0) {
        @VisibleForTesting
        final Consumer<String> applyText =
            output -> {
              if (output != null) {
        ...
        @VisibleForTesting
        final Consumer<String> applyTextPro =
            output -> {
              if (output != null) {
        ...
        if (length < CASE_CONVERSION_SYNC_THRESHOLD) {
          ...
          String convertedText =
              caseType == CaseHandler.CaseType.UPPER
  ...
  public void setEditorLanguage(
      ...
      boolean autoCompleteWindowEnabled,
      boolean isAutoCompleteSymbols,
    ...
    if (lang instanceof EmptyLanguage || !(lang instanceof TextMateLanguage) || !isRefreshing) {
      language =
          createTextMateLanguage(languageScope, autoCompleteWindowEnabled, isAutoCompleteSymbols);
    } else {
      ensureTextmateTheme();
   ...
   *
   * <p>Better option is to recall setEditorLanguage as it completely calls
   * CodeEditor#rerunAnalysis() internally
   *
   * @throws Exception if an error occurs
   */
  ...
  private TextMateLanguage createTextMateLanguage(
      @NonNull String langScope, boolean autoCompleteWindowEnabled, boolean isAutoCompleteSymbols) {
  ...
  public String getMatchingSearchResult(boolean hasBrackets) {
    ...
    if (count == 0) {
      ...
      matchText =
          (count == 1)
              ? 1 + context.getResources().getQuantityString(R.plurals.editor_search_matches, 1)
              : count
                  + context
                      .getResources()
  ...
  private static void loadDefaultEditorLanguages() {
    GrammarRegistry.getInstance().loadGrammars(ContextualCodeEditor.ASSETS_LANGUAGE_GRAMMAR_PATH);
  }
          ...
          () -> {
            try {
              ...
              for (int i = 0; i < length; i += bufferSize) {
                ...
                if (!isProgressBarVisible.get() && elapsedTime(startTime) > PROGRESS_BAR_DELAY_MS) {
