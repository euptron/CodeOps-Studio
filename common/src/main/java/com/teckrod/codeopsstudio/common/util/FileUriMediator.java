...
public class FileUriMediator {
  ...
  private boolean isDirectory, isVirtual = false;

  public static FileUriMediator resolveTree(Uri uri, Context context) {
    String treeDocID = DocumentsContractCompat.getTreeDocumentId(uri);
    Uri treeUri = DocumentsContractCompat.buildDocumentUriUsingTree(uri, treeDocID);
  ...
  public File getFile() throws Exception {
    ...
    if (isDirectory) {
      ...
      switch (authority) {
        ...
        default:
          throw new UnsupportedOperationException("Unsupported URI authority: " + authority);
      }
    }
  }
          ...
          case "home":
            return new File(
                Environment.getExternalStorageDirectory()
                    ...
                    + relativePath);
          default:
            return new File(Environment.getStorageDirectory(), storageType + relativePath);
        }
      }
    }
  ...
  private File handleDownloadStorageAuth() {
    ...
    if (!TextUtils.isEmpty(id)) {
      ...
      for (String segement : segements) {
        try {
          final Uri contentUri = ContentUris.withAppendedId(Uri.parse(segement), Long.valueOf(id));
          File file = new File(getDataColumn(contentUri));
          if (file != null) return file;
        } catch (Exception e) {
          // Ignore because in Android 8 and 9 the id is not a number
          File file =
              new File(uri.getPath().replaceFirst("^/document/raw:", "").replaceFirst("^raw:", ""));
          if (file != null) return file;
        }
      }
    }

    return FileUtil.findInVolumes(Environment.DIRECTORY_DOWNLOADS + File.separator + getFileName());
  }
  ...
  public String getAbsoluteRelativePath() {
    return File.separator + getRelativePath();
  }

  /**
   * Retrieves the path relative to the root directory. This path is independent of the actual root
  ...
  /**
   * Retrieves the display name from the URI. This name is provider-specific and may differ from the
   * actual file name.
   *
  ...
  /**
   * Retrieves the file name from the URI. Falls back to parsing the URI path if the display name is
   * not available.
   *
  ...
  public boolean isAllowedAuthority(String authority) {
    return StorageVolumeAuthority.getAllowedAuthorities().contains(authority);
  }

  static class StorageVolumeAuthority {
    static final String WHATSAPP = "com.whatsapp.provider.media";
    static final String EXTERNAL = "com.android.externalstorage.documents";
    static final String DOWNLOAD = "com.android.providers.downloads.documents";
    static final String MEDIA = "com.android.providers.media.documents";
    static final String GOOGLE_PHOTOS = "com.google.android.apps.photos.content";
    static final String GOOGLE_DRIVE = "com.google.android.apps.docs.storage";
    static final String GOOGLE_DRIVE_LEGACY = "com.google.android.apps.docs.storage.legacy";
    static final String CONTENT = ContentResolver.SCHEME_CONTENT;
    static final String FILE = ContentResolver.SCHEME_FILE;
