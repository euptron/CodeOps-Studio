...
public class TwoDScrollView extends FrameLayout {
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) {
            ...
            if (getChildCount() > 0) {
                View child = getChildAt(0);
                scrollTo(clamp(x,
                    getWidth() - getPaddingRight() - getPaddingLeft(), child.getWidth()), clamp(y,
                    getHeight() - getPaddingBottom() - getPaddingTop(), child.getHeight()));
            } else {
                scrollTo(x, y);
            }
     ...
     *                  finger/curor is moving down the screen, which means we want to scroll
     *                  towards the top.
     */
    public void fling(int velocityX, int velocityY) {
        if (getChildCount() > 0) {
            ...
            if (newFocused != findFocus() && newFocused.requestFocus(
                movingDown ? View.FOCUS_DOWN : View.FOCUS_UP)) {
    ...
    private View findFocusableViewInMyBounds(final boolean topFocus, final int top,
        ...
        if ((preferredFocusable != null) && (preferredFocusable.getTop() < bottomWithoutFadingEdge)
            ...
            preferredFocusable.getLeft() < rightWithoutFadingEdge) && (preferredFocusable.getRight()
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (top < viewBottom && viewTop < bottom && left < viewRight && viewLeft < right) {
                ...
                final boolean viewIsFullyContained =
                    (top < viewTop) && (viewBottom < bottom) && (left < viewLeft) && (viewRight
                        < right);
                if (focusCandidate == null) {
                    ...
                    final boolean viewIsCloserToVerticalBoundary =
                        (topFocus && viewTop < focusCandidate.getTop()) || (!topFocus
                            && viewBottom > focusCandidate.getBottom());
                    final boolean viewIsCloserToHorizontalBoundary =
                        (leftFocus && viewLeft < focusCandidate.getLeft()) || (!leftFocus
    ...
    private boolean canScroll() {
        ...
        if (child != null) {
            ...
            return (getHeight() < childHeight + getPaddingTop() + getPaddingBottom()) || (getWidth()
    ...
    /**
     * If rect is off screen, scroll just enough to get it (or at least the first screen size
     * chunk of
     * it) on screen.
     *
    ...
    private boolean scrollAndFocus(int directionY, int top, int bottom, int directionX, int left,
        ...
        if ((top >= containerTop && bottom <= containerBottom) || (left >= containerLeft
