 ...
 * @author Etido Peter
 */
public class ContextualObserver implements DefaultLifecycleObserver {

  public static final String TAG = ContextualObserver.class.getSimpleName();

  private static final String KEY_PICK_FILE = "ide.pick.file";
  private static final String KEY_CREATE_FILE = "ide.create.file";
  private static final String KEY_SAVE_DOCUMENT_AS = "ide.save.as";
  private static final String KEY_PICK_DIRECTORY = "ide.pick.folder";
  private static final String KEY_REQUEST_DIR_PERMISSION = "ide.request.dir.permission";
  private static final String EXTRA_SAVE_AS = KEY_SAVE_DOCUMENT_AS + "intent.extra.save.as";
  ...
  public ContextualObserver(
      @NonNull Context context,
      @NonNull ActivityResultRegistry registry,
      @NonNull FragmentActivity activity) {
    this.context = Objects.requireNonNull(context, "Context must not be null");
    this.resultRegistry =
        Objects.requireNonNull(registry, "ActivityResultRegistry must not " + "be null");
  ...
  public void onCreate(@NonNull LifecycleOwner owner) {
    ...
    requestFolderPermissionLauncher =
        resultRegistry.register(
            KEY_REQUEST_DIR_PERMISSION,
            owner,
            new ActivityResultContracts.OpenDocumentTree(),
            uri -> {
              if (uri != null) {
  ...
  public void requestDirPermission(final Uri uri, boolean mustBeRoot) {
    ...
    try {
      ...
      notifyError(new IOException(context.getString(R.string.msg_directory_access_config), e));
    }
  }
  ...
  private void mediateFolderUri(Uri uri) {
    ...
    try {
      ...
      notifyError(new IOException(context.getString(R.string.msg_folder_access_violation), e));
    }
  }
  ...
  private void performSaveAs(ActivityResult result) {
    ...
    try {
      ...
      UriUtils.overWriteDocument(
          context, uri, UriUtils.readDocumentToString(context, otherUri, encoding));
