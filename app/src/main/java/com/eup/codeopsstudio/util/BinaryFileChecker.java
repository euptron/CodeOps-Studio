...
public class BinaryFileChecker {
   ...
   *
   * <p>By using the file magic number, detecting the file type is faster because we do not have to
  ...
  /**
   * Tweaks affects performance, larger read bytes improves precision with pefromance as a trade off
   * and vice versa
   */
  ...
  /**
   * Controls how aggressively files are classified as binary.
   *
   * <p>0.0 → very conservative (requires strong evidence) 1.0 → very aggressive (minimal evidence
   * required)
   */
  private static double detectionAggressiveness = 0.0;
   ...
   */
  public static boolean isBinaryFile(File file, boolean useStrictPolicy, final int bufferSize)
      throws IOException {
    ...
    try (InputStream inputStream = new FileInputStream(file)) {
      ...
      if (hasHighEntropy(buffer, bytesRead)) {
        ...
        double weightFactor = Math.min(1.0, sampleSize / 256.0); // 256 = minimum "trusted" size
    ...
    return score >= weightThresholdInterpolation();
  }
  ...
  private static boolean containsBinaryContent(byte[] buffer, final int bytesRead) {
    ...
    for (byte b : buffer) {
      ...
      if (unsigned == HORIZONTAL_TAB
          || unsigned == BACKSPACE
          || unsigned == LINE_FEED
          || unsigned == CARRIAGE_RETURN
          || unsigned == SHIFT_OUT) {
  ...
  private static int weightThresholdInterpolation() {
    double rawThreshold = WEIGHT_THRESHOLD * (1.0 - detectionAggressiveness);
    int adjusted = (int) Math.round(rawThreshold);
    return Math.max(1, adjusted);
  }

  /**
   * Sets the aggressiveness level for binary file detection. Higher aggressiveness = more files
   * will be accepted as non-binary (text).
   *
   * @param aggressivenessValue 0.0 (very strict, minimum tolerance) to 1.0 (very tolerant, maximum
   *     aggressiveness)
   * @throws IllegalArgumentException if tolerance outside [0, 1]
   */
  public static void setAggressiveness(double aggressivenessValue) {
    if (detectionAggressiveness < 0.0 || aggressivenessValue > 1.0) {
      throw new IllegalArgumentException(
          "Aggressiveness must be between 0.0 and 1.0 inclusive. Got: " + aggressivenessValue);
    }
    detectionAggressiveness = aggressivenessValue;
  }

  public static double getAggressiveness() {
    return detectionAggressiveness;
  }
}
