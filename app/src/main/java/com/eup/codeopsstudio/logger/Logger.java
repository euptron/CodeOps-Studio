...
public class Logger {
  ...
  public void postLog(@NonNull String message) {
    ...
    add(new LogModel(highlightNumbers(message)));
  }

  public void d(String tag, String message) {
    if (!isAttached) return;
    ILog.debug(tag, message);
    add(new LogModel(formatDate(), tag, getLogLevel(LogLevel.DEBUG), highlightNumbers(message)));
  }
  ...
  public void w(String tag, String message) {
    ...
    add(new LogModel(formatDate(), tag, highlightSpan(getLogLevel(LogLevel.WARN), 0xffff7043), message));
  }
  ...
  private void add(@NonNull LogModel log) {
    if (logClass == LogClass.BUILD) {
      ArrayList<LogModel> currentList = model.getBUILDLogs().getValue();
      if (currentList == null) currentList = new ArrayList<>();

      currentList.add(log);
      model.getBUILDLogs().postValue(currentList);
    } else if (logClass == LogClass.IDE) {
      ArrayList<LogModel> currentList = model.getIDELogs().getValue();
      if (currentList == null) currentList = new ArrayList<>();
  ...
  private SpannableString highlightSpan(@NonNull String message, int color) {
    SpannableString spannableMessage = new SpannableString(message);
    spannableMessage.setSpan(
        new ForegroundColorSpan(color), 0, message.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    return spannableMessage;
  }
