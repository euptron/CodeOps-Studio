...
public class PaneFactoryImpl implements PaneFactory {
    ...
    private static final Type GSON_TYPE_LIST_MAP = new TypeToken<List<LinkedTreeMap<String,
        Object>>>() { }.getType();
    ...
    public Pane createPane(
        @NonNull Map<String, Object> arguments) throws UnsupportedOperationException {
    ...
    public UUID getID(@NonNull JSONObject json) {
        try {
            return json.has("uuid") && !json.isNull("uuid")
                ? UUID.fromString(json.getString("uuid")) : null;
        } catch (JSONException e) {
     ...
     * @return the boolean value paired with the map key or false if the assigned key does not
     * exist.
     ...
     * @param map the map holding the key-value pair
     * @return the string value paired with the map key or an empty string if the assigned key does
     * not exist.
     * @see #requireInt(String, Map)
    ...
    private Pane createCodeEditorPane(@NonNull Map<String, Object> data) {
        ...
        final long fileMTime = requireLong(CodeEditorPane.KEY_FILE_TIME, data);
        ...
        codeEditorPane.addArguments(CodeEditorPane.KEY_FILE_TIME, fileMTime);
    ...
    public static int requireInt(@NonNull String key, @NonNull Map<String, ?> map) {
        ...
        if (object instanceof Integer o) {
            ...
            ILog.debug(TAG,
                "Expected numeric or string type for int, but got: " + (object != null ? object
    ...
    public static long requireLong(@NonNull String key, @NonNull Map<String, ?> map) {
        ...
        if (object instanceof Long o) {
            ...
            ILog.debug(TAG,
                "Expected numeric or string type for long, but got: " + (object != null ? object
                    .getClass().getName() : "null"));
