...
/**
 * TODO: defer heavy task like loading the treeview because current impl causes lags since ui is not
 * ye laid out and we try to load the treeview data, also work on a new treeview
 */
public class TreeViewFragment extends Fragment
    implements TreeNode.TreeNodeClickListener,
  ...
  public void onFileChanged(int event, String path) {
    ...
    switch (event) {
      ...
      case FileObserver.MOVE_SELF:
        ...
        break;
      case FileObserver.MOVED_FROM, FileObserver.MOVED_TO, FileObserver.CREATE, FileObserver.DELETE:
  ...
  private void chooseTemplates() {
    BaseUtil.toastShort(R.string.coming_soon);
  }
  ...
  private void handleOptionClick(
      ActionModel model, File file, @Nullable TreeNode node, boolean isRoot) {
    if (!file.exists()) {
      ILog.warning(
          TAG, "Cannot handle options file/folder " + file.getAbsolutePath() + "does not exist");
      return;
    }
  ...
  private void populateFileTree(File dir) {
    ...
    if (!Wizard.isEmpty(lastOpenedFilePath) && !lastOpenedFilePath.equals(dir.getAbsolutePath())) {
  ...
  private void showOptionsBottomSheet(@NonNull File file, @Nullable TreeNode node, boolean isRoot) {
   ...
   *
   * <p>The current tree state is preserved before repopulating, so that any expanded or selected
   * nodes can be restored after the update.
   *
